...
x2 = x2.replace("", ".*"); if(x1.matches(x2))
      return true;
else
      return false;
}
...
for(String y:al)
      if(i == y.length())
            {
                  double count = 0;
for(String x:bl) if(check(x,y))
                              count++;
                  if(count/bl.size() >= support)
{
if(i == 4)
} }
cl.add(y); System.out.println(y + " ->
      "+count/bl.size());
}
System.out.println("\n----------Strong rules------------\n"); //generate rules
for(String p:cl)
...
{
if(check(x,p))
count1++; if(check(x,temp3))
count2++;
                        }
                        if(count2 > 0 && (count1/count2) >=
confidence) System.out.println(temp3+"->"+temp4+"
